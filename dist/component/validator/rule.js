...
addRule('required', function (val, element, rule, ctx) {
    var type = null;
...
addRule('min', function (val, element, rule, ctx) {
    return Number(val) >= Number(ctx[rule]);
}, '{{name}}必须大于或等于{{min}}');

addRule('max', function (val, element, rule, ctx) {
    return Number(val) <= Number(ctx[rule]);
}, '{{name}}必须小于或等于{{max}}');

addRule('minlength', function (val, element, rule, ctx) {
    return (val || '').toString().length >= Number(ctx[rule]);
}, '{{name}}的长度必须大于或等于{{minlength}}');

addRule('maxlength', function (val, element, rule, ctx) {
    return (val || '').toString().length <= Number(ctx[rule]);
...
function addRule(name, operator, message) {
    ...
    if (operator instanceof RegExp) {
        rules[name] = [function (val, element, rule, ctx) {
